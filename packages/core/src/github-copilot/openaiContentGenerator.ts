      ...
      for (const message of messages) {
        if (
          ...
          const validToolCalls = message.tool_calls.filter(
            (toolCall: OpenAI.Chat.ChatCompletionMessageToolCall) => toolCall.id && toolResponseIds.has(toolCall.id),
          );
      ...
      for (const message of cleaned) {
        if (
          ...
          const finalValidToolCalls = message.tool_calls.filter(
            (toolCall: OpenAI.Chat.ChatCompletionMessageToolCall) => toolCall.id && finalToolResponseIds.has(toolCall.id),
          );
    ...
    private cleanOrphanedToolCallsForLogging(
      ...
      for (const message of messages) {
        if (message.role === 'assistant' && message.tool_calls) {
          // Filter out tool calls that don't have corresponding responses
          const validToolCalls = message.tool_calls.filter(
            (toolCall: OpenAIToolCall) => toolCall.id && toolResponseIds.has(toolCall.id),
          );
      ...
      for (const message of cleaned) {
        if (message.role === 'assistant' && message.tool_calls) {
          const finalValidToolCalls = message.tool_calls.filter(
            (toolCall: OpenAIToolCall) => toolCall.id && finalToolResponseIds.has(toolCall.id),
          );
