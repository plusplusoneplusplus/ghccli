...
import { GlobTool, GlobToolParams, GlobToolInvocation, GlobPath, sortFileEntries } from './glob_v2.js';
...
describe('GlobTool', () => {
  ...
  describe('execute', () => {
    it('should find files matching a simple pattern in the root', async () => {
      ...
      const invocation = globTool.createInvocation(params);
  ...
  describe('validateToolParams', () => {
    it('should return null for valid parameters (pattern only)', () => {
      ...
      const invocation = globTool.createInvocation(params) as GlobToolInvocation;
    ...
    it('should return error if pattern is missing (schema validation)', () => {
      ...
      expect(() => globTool.validateAndCreateInvocation(params)).toThrow();
    });
    ...
    it("should return error if search path resolves outside the tool's root directory", () => {
      ...
      const invocation = specificGlobTool.createInvocation(paramsOutside) as GlobToolInvocation;
      expect(invocation.validateToolParams(paramsOutside)).toContain(
    ...
    it('should return error for non-positive limit', () => {
      ...
      const invocation = globTool.createInvocation(paramsZero) as GlobToolInvocation;
      expect(invocation.validateToolParams(paramsZero)).toBe(
        'Limit must be a positive number.',
      );

      const paramsNegative: GlobToolParams = { pattern: '*.txt', limit: -5 };
      const invocation2 = globTool.createInvocation(paramsNegative) as GlobToolInvocation;
      expect(invocation2.validateToolParams(paramsNegative)).toBe(
        'Limit must be a positive number.',
      );
    });
    ...
    it('should return error for limit exceeding hard maximum of 500', () => {
      ...
      const invocation = globTool.createInvocation(params) as GlobToolInvocation;
      expect(invocation.validateToolParams(params)).toBe(
      ...
      const invocation2 = globTool.createInvocation(paramsWayTooHigh) as GlobToolInvocation;
      expect(invocation2.validateToolParams(paramsWayTooHigh)).toBe(
        'Limit cannot exceed 500 files to prevent overwhelming output.',
      );
    });
